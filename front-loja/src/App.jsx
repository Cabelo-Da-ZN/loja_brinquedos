...
import CarrinhoLateral from './components/Carrinho'; 
import Swal from 'sweetalert2';

// COMPONENTE DE SELETOR DE CATEGORIAS
function SeletorCategorias({ onFilter }) {
  ...
  return (
    <div className="container py-5 mt-2"> 
      <div className="d-flex justify-content-center align-items-start gap-5 overflow-auto pb-4" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none', minHeight: '220px' }}>
        {filtrosReais.map((f) => (
          <div key={f.id} className="text-center" style={{ cursor: 'pointer', minWidth: '160px', paddingTop: '10px' }} onClick={() => onFilter(f.id)}>
            <div className="rounded-circle shadow-lg mb-3 d-flex align-items-center justify-content-center"
              style={{ width: '150px', height: '150px', backgroundColor: '#fff', overflow: 'hidden', position: 'relative', transition: 'transform 0.4s', margin: '0 auto' }}
              onMouseOver={(e) => e.currentTarget.style.transform = 'scale(1.15)'}
              onMouseOut={(e) => e.currentTarget.style.transform = 'scale(1)'}>
              <img src={f.img} alt={f.label} style={{ width: '100%', height: '100%', objectFit: 'cover', transform: 'scale(1.35)' }}
                onError={(e) => e.target.src = 'https://via.placeholder.com/150/ff007f/ffffff?text=' + f.label} />
            </div>
            <p className="fw-bold text-dark text-uppercase m-0" style={{ fontSize: '14px', letterSpacing: '1px' }}>{f.label}</p>
          </div>
        ))}
      </div>
    </div>
    ...
    <div className="row row-cols-1 row-cols-md-4 g-4">
      {brinquedos.map(b => (
        <div key={b.id} className="col">
          <div className="card h-100 border-0 shadow-sm rounded-4 p-3 text-center" style={{ cursor: 'pointer', transition: 'transform 0.2s' }}
               onMouseEnter={(e) => e.currentTarget.style.transform = 'scale(1.03)'}
               onMouseLeave={(e) => e.currentTarget.style.transform = 'scale(1)'}
               onClick={() => navigate(`/produto/${b.id}`)}>
            <img src={b.caminhoImagem.startsWith('http') ? b.caminhoImagem : `http://localhost:8080/imagens/${b.caminhoImagem}`} alt={b.nome} className="img-fluid mb-2" style={{height: '180px', objectFit: 'contain'}} />
            ...
            <button className="btn btn-success btn-sm rounded-pill fw-bold w-100" onClick={(e) => { e.stopPropagation(); adicionarAoCarrinho(b); }}>Adicionar 🛒</button>
          </div>
        </div>
      ))}
    </div>
...
function App() {
  ...
  const carregarBrinquedos = (termo = "") => {
    const url = termo ? `http://localhost:8080/api/brinquedos/buscar?termo=${termo}` : "http://localhost:8080/api/brinquedos";
  ...
  const alterarQuantidade = (id, delta) => {
    setCarrinho(atual => atual.map(i => i.id === id ? { ...i, quantidade: Math.max(1, i.quantidade + delta) } : i));
  };
  ...
  return (
    <Router>
      ...
      <Routes>
        ...
        <Route path="/" element={
          <div className="loja-layout min-vh-100 bg-light">
            ...
            <div className="container py-5">
              <h2 className="mb-4 fw-bold text-secondary text-center">Brinquedos em <span style={{ color: '#ff007f' }}>Destaque</span> 🧸</h2>
        ...
        {/* NOVA ROTA: PÁGINA DE CARRINHO CORRIGIDA */}
        <Route path="/carrinho" element={
          <div className="loja-layout min-vh-100 bg-light">
            <Header onSearch={carregarBrinquedos} onFilter={filtrarPorCategoria} carrinho={carrinho} />
            <div className="container py-5 text-center" style={{ minHeight: '60vh' }}>
               <h2 className="fw-bold mb-4">Seu Carrinho de Compras</h2>
               <p className="text-muted">Clique no ícone de carrinho no menu superior para ver seus itens e finalizar o pedido!</p>
            </div>
            <Footer />
          </div>
        } />

        <Route path="/produto/:id" element={<Detalhes adicionarAoCarrinho={adicionarAoCarrinho} carrinho={carrinho} onSearch={carregarBrinquedos} onFilter={filtrarPorCategoria} />} />
